...
import { Card, CardActions, CardBody, CardDescription, CardFooter, CardImage, CardTitle } from '@project/common/components/card';
...
import * as mixins from '@project/common/styles/mixins.css'
import * as styles from './styles.css';

const Button: React.FC<React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>> = ({ children, className, ...rest }) => (
    ...
    return (
        <Card>
            ...
            <CardFooter>
                ...
                <CardActions>
                    {inCart
                        ? <Button className={mixins.primary} onClick={removeFromCart}>Remove</Button>
                        : <Button className={mixins.primary} onClick={addToCart}>Add</Button>
                    }
                </CardActions>
            </CardFooter>
        </Card>
...
const ProductsList = memo(() => {
    ...
    return (
        <>
            {(operation?.result || []).map(product => <ProductCard key={product.id} product={product} />)}
        </>
    )
})
